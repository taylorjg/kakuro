...
class KakuroConstraint extends Constraint {
  ...
  satisfied(assignment) {
    ...
    const sumOfValues = U.sum(values)
...
function* findRunDigits(n, requiredTotal) {
  ...
  function* helper(n, useds = [], array = []) {
    const remainingDigits = digitsWithout(U.flatten(useds))
    const used = []
    useds.push(used)
...
const findRuns = (puzzle, clues, rowIncrement, colIncrement) => {
  return clues.map(([row, col, sum]) => {
    ...
    const digits = Array.from(new Set(U.flatten(tuples))).sort((a, b) => a - b)
...
const main = puzzle => {
  ...
  const allRuns = U.flatten([acrossRuns, downRuns])
